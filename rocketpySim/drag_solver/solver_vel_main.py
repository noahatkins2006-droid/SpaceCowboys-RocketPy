...
from solver import TimeBasedVelocity
...
def f(x,startTime,endTime):
    ...
    vel_data = TimeBasedVelocity(open((Path(__file__).resolve().parent/"helios_solver_data"/"helios_time_x_velocity.csv").resolve(),"r"), vel_df)
    total_deviation = vel_data.get_total_deviation(startTime,endTime)
    abs_deviation = vel_data.get_absolute_deviation(startTime,endTime)
